...
const Container = styled('div')<ContainerProps>(({ menuOpen, theme: { palette, breakpoints } }) => ({
...
const ChatListContainer = styled('div')(({ theme: { breakpoints } }) => ({
...
const ListButton = styled(Button)(({ theme: { breakpoints } }) => ({
  width: 180,
  height: 32,
  padding: 0,
  [breakpoints.down(800)]: {
...
function ConversationList({
  ...
  return (
    <Container menuOpen={menuOpen}>
      ...
      <ButtonContainer>
        <ListButton variant="contained" onClick={() => dispatch(navigate({ to: PageType.SETTING_GLOBAL }))}>
          Setting
        </ListButton>
      ...
      {onMobile || (
        <ChatListContainer>
          {chatOrder.map((chatId) => (
            <ChatTitleBox key={chatId} chatId={chatId} isCurrent={chatId === currentChatId} />
          ))}
        </ChatListContainer>
      )}
    </Container>
