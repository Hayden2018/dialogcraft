...
const ChatContainer = styled('div')(({ theme: { breakpoints } }) => ({
...
const HeaderBanner = styled('div')(({ theme: { palette, breakpoints } }) => ({
...
const ChatTitle = styled('div')(({ theme: { breakpoints } }) => ({
  fontSize: 20,
  width: 'calc(100% - 15px)',
  lineHeight: '32px',
  whiteSpace: 'nowrap',
...
const HeaderButton = styled(Button)(({ theme: { breakpoints } }) => ({
  width: 160,
  height: 32,
  [breakpoints.down(600)]: {
    width: '100%',
...
const StopGenerationButton = styled(Button)(({ theme: { breakpoints } }) => ({
  gridColumn: '2 / 4',
  justifySelf: 'right',
  width: 200,
  height: 32,
...
const MessageArea = styled('div')<AreaProps>(({ theme, isEditing }) => ({
  overflowY: 'scroll',
...
const DraftGrid = styled('div')(({ theme: { breakpoints } }) => ({
  height: 102,
  margin: '10px',
  display: 'grid',
...
const MessageInput = styled(TextField)(({ theme: { breakpoints } }) => ({
  gridColumn: '1',
  gridRow: '1 / span 2',
  [breakpoints.down(800)]: {
    gridColumn: '1 / 4',
...
const SendButton = styled(Button)(({ theme: { breakpoints } }) => ({
  width: 165,
  height: 45,
  gridColumn: '2',
  gridRow: '1',
...
const RegenerateButton = styled(Button)(({ theme: { breakpoints } }) => ({
  width: 165,
  height: 45,
  gridColumn: '2',
  gridRow: '2',
...
const MenuIcon = styled(menuIcon)(({ theme }) => ({
  width: 30,
  height: 30,
...
const NoChatIcon = styled(noChatIcon)(({ theme }) => ({
  display: 'block',
  verticalAlign: 'top',
  width: 260,
  height: 260,
...
const StartNewChatButton = styled(Button)(({ theme }) => ({
  display: 'block',
  width: 200,
  height: 40,
  margin: '20px auto',
...
const NoChatInfo = styled('p')(({ theme: { palette } }) => ({
  textAlign: 'center',
...
function ChatInterface({ setMenuOpen }: { setMenuOpen: React.Dispatch<React.SetStateAction<boolean>> }) {
  ...
  const { onChange, sendMessage, regenerate, onKeyDown, stopGenerate, draft } =
    useMessageActions(currentChat);
    ...
    return (
      <ChatContainer>
        <HeaderBanner>
          ...
          {isStreaming ? (
            <StopGenerationButton color="error" variant="contained" onClick={stopGenerate}>
              Stop Generation
            ...
            <>
              ...
              <HeaderButton variant="contained" color={editing ? 'success' : 'warning'} onClick={toggleEdit}>
