...
function validateImportData(
  ...
  for (const chatId of chatOrder) {
    ...
    if (chat.id !== chatId || typeof chat.title !== 'string') throw Error('Invalid format');
...
function handleChatImport(
  ...
  try {
    ...
    const { version, chatList: importedChatList, chats: importedChats } = window.require(filePath);
    ...
    mergedChatOrder.sort((chatIdA: string, chatIdB: string) => {
