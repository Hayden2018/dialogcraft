...
const CodeBlockHeader = styled('div')(({ theme: { palette } }) => ({
  fontSize: 16,
...
const CopyButton = styled(Button)(({ theme: { palette }, color }) => ({
  position: 'absolute',
  right: 10,
  padding: '0px 14px',
  width: 'fit-content',
...
function CustomCodeBlock({ language, code, style = {} }: { language: string; code: string; style?: object }) {
  ...
  return (
    <div style={{ minWidth: 200, marginBottom: 8, ...style }}>
      <CodeBlockHeader>
        {language || ' '}
        <CopyButton variant="contained" onClick={handleCopy} color={copySuccess ? 'success' : 'primary'}>
