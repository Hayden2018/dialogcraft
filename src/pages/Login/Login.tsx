...
const GreetingContainer = styled('div')(({ theme }) => ({
  height: '100%',
  width: '100%',
  display: 'flex',
...
const AppTitle = styled('h1')(({ theme: { palette } }) => ({
  margin: '10px 0px 5px',
  textAlign: 'center',
...
const CredentialInput = styled(TextField)(({ theme }) => ({
  maxWidth: '580px',
  width: '100%',
  margin: '0px auto',
}));

const InfoText = styled('p')(({ theme: { palette, breakpoints } }) => ({
  margin: '0px auto',
  textAlign: 'center',
...
export default function Login() {
  ...
  if (status === SettingStatus.VERIFYING)
    return (
      <GreetingContainer>
        <ProgressContainer>
    ...
    <form onSubmit={handleSubmit(onSubmit)} id="app">
      <GreetingContainer>
        ...
        <InfoText>
          ...
          <Link target={openTarget} href={openRouterKeysUrl}>
            openrouter.ai/keys
          </Link>
        ...
        {onElectronEnv() || (
          <InfoText>
            You are using the web version of DialogCraft. More features available on{' '}
            <Link target={openTarget} href={releaseUrl}>
              desktop app
            </Link>
            .
          </InfoText>
        ...
        <InfoText>
          Your API key will be stored on this device. This application does not interact with systems other
