...
const Form = styled('form')(({ theme: { breakpoints } }) => ({
  margin: 'auto',
  padding: '0px 20px',
  maxWidth: 1000,
...
const FormHeader = styled('div')(({ theme: { palette } }) => ({
  backgroundColor: palette.mode === 'dark' ? '#383838' : '#fdfdfd',
  zIndex: 100,
  textAlign: 'center',
  fontSize: 30,
...
const FormRow = styled('div')<RowProps>(({ theme, narrow, tall }) => ({
...
const SliderTop = styled('div')(({ theme }) => ({
  display: 'flex',
  justifyContent: 'space-between',
  fontSize: 17,
}));

const SubmitRow = styled('div')(({ theme }) => ({
  ...
  return (
    <Container>
      <Form onSubmit={handleSubmit(onSubmit)}>
        ...
        <FormRow>
          <TextField disabled fullWidth label="API Base URL" {...register('baseURL')} />
        </FormRow>
        <FormRow>
          <TextField disabled fullWidth type="password" label="OpenRouter API Key" {...register('apiKey')} />
        </FormRow>
        <ActionRow>
        ...
        <FormRow>
          <Alert severity="info">
            <InfoList>
              ...
              <li>
                Temperature - higher values will make the output more random, while lower values more
                deterministic.
              </li>
              <li>Top P - the model only considers tokens within top P probability mass.</li>
              <li>
                Reasoning effort - controls how much thinking budget reasoning models use. Set to None for
                non-reasoning models.
              </li>
              <li>
                Exclude reasoning - model may still reason internally, but the thinking trace is not returned.
              </li>
            </InfoList>
          </Alert>
        </FormRow>
        <FormRow>
          ...
          >
            {globalSettings.availableModels.map((modelId, index) => (
              <MenuItem value={modelId} key={index}>
                {modelId}
              </MenuItem>
            ))}
          </Select>
        ...
        <FormRow tall>
          <Alert severity="info">
            <InfoList>
              <li>
                System Prompt - guidance for the model on the conversation context before any user messages
              </li>
              <li>
                Maximum context messages - maximum number of previous messages visible by the model. Lowering
                this number help reduce API cost but the model may forget earlier conversations.
              </li>
            </InfoList>
          </Alert>
        </FormRow>
        <FormRow>
          <TextField label="System Prompt" fullWidth multiline rows={4} {...register('systemPrompt')} />
        </FormRow>
        <FormRow narrow tall>
          <SliderTop>
        ...
        <FormRow>
          <Alert severity="info">
            <li>
              Press <b>{enterSend ? 'Enter' : 'Shift + Enter'}</b> to send your message.
            </li>
            <li>
              Press <b>{enterSend ? 'Shift + Enter' : 'Enter'}</b> to add a new line in your message.
            </li>
            {autoTitle ? (
